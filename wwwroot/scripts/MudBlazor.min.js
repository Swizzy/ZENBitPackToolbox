...
class MudElementReference {
    ...
    selectRange(element, pos1, pos2) {
        if (element) {
            if (element.createTextRange) {
                ...
                selRange.select();
            } else element.setSelectionRange ? element.setSelectionRange(pos1, pos2) : element.selectionStart && ((element.selectionStart = pos1), (element.selectionEnd = pos2));
            element.focus();
        }
    }
    select(element) {
    ...
    addDefaultPreventingHandler(element, eventName) {
        ...
        return element.addEventListener(eventName, listener, { passive: !1 }), (this.eventListeners[++this.listenerId] = listener), this.listenerId;
    }
...
class MudThrottledEventManager {
    ...
    subscribe(eventName, elementId, projection, throotleInterval, key, properties, dotnetReference) {
        ...
        if (elem) {
            ...
            if (projection) {
                ...
                if (parts.length == 0 || parts.length == 1) functionPointer = functionPointer[projection];
                else
                    for (let i = 0; i < parts.length; i++)
                        if (((functionPointer = functionPointer[parts[i]]), !functionPointer)) {
    ...
    subscribeGlobal(eventName, throotleInterval, key, properties, dotnetReference) {
        ...
        this.mapper[key] = { eventName: eventName, handler: handlerRef, delay: throotleInterval, timerId: -1, reference: dotnetReference, elementId: document, properties: properties, projection: null };
    }
    throttleEventHandler(key, event) {
        const entry = this.mapper[key];
        entry && (clearTimeout(entry.timerId), (entry.timerId = window.setTimeout(this.eventHandler.bind(this, key, event), entry.delay)));
    }
    eventHandler(key, event) {
        var elem, i;
        const entry = this.mapper[key];
        if (entry && ((elem = document.getElementById(entry.elementId)), elem == event.srcElement || entry.elementId == document)) {
            const eventEntry = {};
            for (i = 0; i < entry.properties.length; i++) eventEntry[entry.properties[i]] = event[entry.properties[i]];
    ...
    unsubscribe(key) {
        const entry = this.mapper[key];
        if (entry) {
            if (((entry.reference = null), document == entry.elementId)) document.removeEventListener(entry.eventName, entry.handler, !1);
            else {
...
window.getTabbableElements = (element) =>
    element.querySelectorAll(
        "a[href]:not([tabindex='-1']),area[href]:not([tabindex='-1']),button:not([disabled]):not([tabindex='-1']),input:not([disabled]):not([tabindex='-1']):not([type='hidden']),select:not([disabled]):not([tabindex='-1']),textarea:not([disabled]):not([tabindex='-1']),iframe:not([tabindex='-1']),details:not([tabindex='-1']),[tabindex]:not([tabindex='-1']),[contentEditable=true]:not([tabindex='-1']"
    );
window.serializeParameter = (data, spec) => {
    ...
    for (let i in data) {
        ...
        if (typeof currentMember != "function" && currentMember !== null) {
            ...
            if (spec != "*") {
                if (((currentMemberSpec = Array.isArray(data) ? spec : spec[i]), !currentMemberSpec)) continue;
            } else currentMemberSpec = "*";
            if (typeof currentMember == "object")
                ...
                } else res[i] = currentMember.length === 0 ? [] : this.serializeParameter(currentMember, currentMemberSpec);
            else currentMember === Infinity && (currentMember = "Infinity"), currentMember !== null && (res[i] = currentMember);
        }
    }
    return res;
};
class MudJsEventFactory {
    connect(dotNetRef, elementId, options) {
        if (!elementId) throw "[MudBlazor | JsEvent] elementId: expected element id!";
        var element = document.getElementById(elementId);
        if (!element) throw "[MudBlazor | JsEvent] no element found for id: " + elementId;
    ...
    subscribe(elementId, eventName) {
        ...
        var element = document.getElementById(elementId);
        if (!element) throw "[MudBlazor | JsEvent] no element found for id: " + elementId;
        if (!element.mudJsEvent) throw "[MudBlazor | JsEvent] please connect before subscribing";
...
class MudJsEvent {
    ...
    connect(element) {
        if (this._options) {
            ...
            if (!this._observer) {
                ...
                this._observer.observe(this._element, { attributes: !1, childList: !0, subtree: !0 });
    ...
    disconnect() {
        if (this._observer) {
            ...
            this._observer = null;
            for (const child of this._observedChildren) this.detachHandlers(child);
        }
    }
    subscribe(eventName) {
        if (!this._subscribedEvents[eventName]) {
            ...
            for (const child of element.getElementsByClassName(targetClass)) this.attachHandlers(child);
        }
    }
    unsubscribe(eventName) {
        if (this._observer) {
            ...
            for (const child of this._observedChildren) this.detachHandler(child, eventName);
        }
    }
    attachHandlers(child) {
        child.mudJsEvent = this;
        for (var eventName of Object.getOwnPropertyNames(this._subscribedEvents))
            this._subscribedEvents[eventName] && (this.logger("[MudBlazor | JsEvent] attaching event " + eventName, child), child.addEventListener(eventName, this.eventHandler));
    ...
    detachHandlers(child) {
        ...
        for (var eventName of Object.getOwnPropertyNames(this._subscribedEvents)) this._subscribedEvents[eventName] && child.removeEventListener(eventName, this.eventHandler);
    ...
    onDomChanged(mutationsList) {
        ...
        for (const mutation of mutationsList) {
            for (const element of mutation.addedNodes) element.classList && element.classList.contains(targetClass) && ((self._options.TagName && element.tagName != self._options.TagName) || self.attachHandlers(element));
            for (const element of mutation.removedNodes) element.classList && element.classList.contains(targetClass) && ((self._options.tagName && element.tagName != self._options.tagName) || self.detachHandlers(element));
        }
    }
    eventHandler(e) {
        var self = this.mudJsEvent,
...
class MudKeyInterceptor {
    ...
    connect(element) {
        if (this._options) {
            ...
            if (!this._observer) {
                ...
                for (const child of this._element.getElementsByClassName(targetClass)) this.attachHandlers(child);
            }
        }
    }
    setKeyOption(keyOption) {
        keyOption.key.length > 2 && keyOption.key.startsWith("/") && keyOption.key.endsWith("/")
            ? ((keyOption.regex = new RegExp(keyOption.key.substring(1, keyOption.key.length - 1))), this._regexOptions.push(keyOption))
    ...
    attachHandlers(child) {
        (this.logger("[MudBlazor | KeyInterceptor] attaching handlers ", { child }), this._observedChildren.indexOf(child) > -1) ||
    ...
    onDomChanged(mutationsList) {
        var self = this.mudKeyInterceptor,
            targetClass = self._options.targetClass;
        for (const mutation of mutationsList) {
            for (const element of mutation.addedNodes) element.classList && element.classList.contains(targetClass) && self.attachHandlers(element);
            for (const element of mutation.removedNodes) element.classList && element.classList.contains(targetClass) && self.detachHandlers(element);
        }
    }
    matchesKeyCombination(option, args) {
        var combi;
        if (!option || option === "none") return !1;
        ...
        return any && option === "key+any" ? !0 : !any && option.includes("key+none") ? !0 : any ? ((combi = `key${shift ? "+shift" : ""}${ctrl ? "+ctrl" : ""}${alt ? "+alt" : ""}${meta ? "+meta" : ""}`), option.includes(combi)) : !1;
    }
    onKeyDown(args) {
        var self = this.mudKeyInterceptor,
        ...
        invoke = !1;
        self._keyOptions.hasOwnProperty(key) &&
            ((keyOptions = self._keyOptions[key]), self.logger('[MudBlazor | KeyInterceptor] options for "' + key + '"', keyOptions), self.processKeyDown(args, keyOptions), keyOptions.subscribeDown && (invoke = !0));
        for (const keyOptions of self._regexOptions)
            keyOptions.regex.test(key) && (self.logger('[MudBlazor | KeyInterceptor] regex options for "' + key + '"', keyOptions), self.processKeyDown(args, keyOptions), keyOptions.subscribeDown && (invoke = !0));
        invoke && ((eventArgs = self.toKeyboardEventArgs(args)), (eventArgs.Type = "keydown"), self._dotNetRef.invokeMethodAsync("OnKeyDown", eventArgs));
    }
    ...
    onKeyUp(args) {
        ...
        invoke = !1;
        self._keyOptions.hasOwnProperty(key) && ((keyOptions = self._keyOptions[key]), self.processKeyUp(args, keyOptions), keyOptions.subscribeUp && (invoke = !0));
        for (const keyOptions of self._regexOptions) keyOptions.regex.test(key) && (self.processKeyUp(args, keyOptions), keyOptions.subscribeUp && (invoke = !0));
        invoke && ((eventArgs = self.toKeyboardEventArgs(args)), (eventArgs.Type = "keyup"), self._dotNetRef.invokeMethodAsync("OnKeyUp", eventArgs));
    }
    ...
    toKeyboardEventArgs(args) {
        return { Key: args.key, Code: args.code, Location: args.location, Repeat: args.repeat, CtrlKey: args.ctrlKey, ShiftKey: args.shiftKey, AltKey: args.altKey, MetaKey: args.metaKey };
    }
}
window.mudpopoverHelper = {
    calculatePopoverPosition: function (list, boundingRect, selfRect) {
        let top = 0,
            left = 0;
        list.indexOf("mud-popover-anchor-top-left") >= 0
            ? ((left = boundingRect.left), (top = boundingRect.top))
            : list.indexOf("mud-popover-anchor-top-center") >= 0
                ? ((left = boundingRect.left + boundingRect.width / 2), (top = boundingRect.top))
                : list.indexOf("mud-popover-anchor-top-right") >= 0
                    ? ((left = boundingRect.left + boundingRect.width), (top = boundingRect.top))
                    : list.indexOf("mud-popover-anchor-center-left") >= 0
                        ? ((left = boundingRect.left), (top = boundingRect.top + boundingRect.height / 2))
                        : list.indexOf("mud-popover-anchor-center-center") >= 0
                            ? ((left = boundingRect.left + boundingRect.width / 2), (top = boundingRect.top + boundingRect.height / 2))
                            : list.indexOf("mud-popover-anchor-center-right") >= 0
                                ? ((left = boundingRect.left + boundingRect.width), (top = boundingRect.top + boundingRect.height / 2))
                                : list.indexOf("mud-popover-anchor-bottom-left") >= 0
                                    ? ((left = boundingRect.left), (top = boundingRect.top + boundingRect.height))
                                    : list.indexOf("mud-popover-anchor-bottom-center") >= 0
                                        ? ((left = boundingRect.left + boundingRect.width / 2), (top = boundingRect.top + boundingRect.height))
                                        : list.indexOf("mud-popover-anchor-bottom-right") >= 0 && ((left = boundingRect.left + boundingRect.width), (top = boundingRect.top + boundingRect.height));
        let offsetX = 0,
            offsetY = 0;
        return (
            list.indexOf("mud-popover-top-left") >= 0
                ? ((offsetX = 0), (offsetY = 0))
                : list.indexOf("mud-popover-top-center") >= 0
                    ? ((offsetX = -selfRect.width / 2), (offsetY = 0))
                    : list.indexOf("mud-popover-top-right") >= 0
                        ? ((offsetX = -selfRect.width), (offsetY = 0))
                        : list.indexOf("mud-popover-center-left") >= 0
                            ? ((offsetX = 0), (offsetY = -selfRect.height / 2))
                            : list.indexOf("mud-popover-center-center") >= 0
                                ? ((offsetX = -selfRect.width / 2), (offsetY = -selfRect.height / 2))
                                : list.indexOf("mud-popover-center-right") >= 0
                                    ? ((offsetX = -selfRect.width), (offsetY = -selfRect.height / 2))
                                    : list.indexOf("mud-popover-bottom-left") >= 0
                                        ? ((offsetX = 0), (offsetY = -selfRect.height))
                                        : list.indexOf("mud-popover-bottom-center") >= 0
                                            ? ((offsetX = -selfRect.width / 2), (offsetY = -selfRect.height))
                                            : list.indexOf("mud-popover-bottom-right") >= 0 && ((offsetX = -selfRect.width), (offsetY = -selfRect.height)),
    ...
    flipClassReplacements: {
        ...
        "top-and-left": { "mud-popover-top-left": "mud-popover-bottom-right" },
        "top-and-right": { "mud-popover-top-right": "mud-popover-bottom-left" },
        "bottom-and-left": { "mud-popover-bottom-left": "mud-popover-top-right" },
        "bottom-and-right": { "mud-popover-bottom-right": "mud-popover-top-left" },
    },
    flipMargin: 0,
    ...
    placePopover: function (popoverNode, classSelector) {
        if (popoverNode && popoverNode.parentNode) {
            ...
            if (popoverContentNode.classList.contains("mud-popover-open") == !1) return;
            if (classSelector && popoverContentNode.classList.contains(classSelector) == !1) return;
            ...
            if (classList.contains("mud-popover-overflow-flip-onopen") || classList.contains("mud-popover-overflow-flip-always")) {
                ...
                if (
                    (selector ||
                        (classList.contains("mud-popover-top-left")
                            ? deltaBottom < graceMargin && deltaToRight < graceMargin && spaceToTop >= selfRect.height && deltaToLeft >= selfRect.width
                                ? (selector = "top-and-left")
                                : deltaBottom < graceMargin && spaceToTop >= selfRect.height
                                    ? (selector = "top")
                                    : deltaToRight < graceMargin && deltaToLeft >= selfRect.width && (selector = "left")
                            : classList.contains("mud-popover-top-center")
                                ? deltaBottom < graceMargin && spaceToTop >= selfRect.height && (selector = "top")
                                : classList.contains("mud-popover-top-right")
                                    ? deltaBottom < graceMargin && deltaToLeft < graceMargin && spaceToTop >= selfRect.height && deltaToRight >= selfRect.width
                                        ? (selector = "top-and-right")
                                        : deltaBottom < graceMargin && spaceToTop >= selfRect.height
                                            ? (selector = "top")
                                            : deltaToLeft < graceMargin && deltaToRight >= selfRect.width && (selector = "right")
                                    : classList.contains("mud-popover-center-left")
                                        ? deltaToRight < graceMargin && deltaToLeft >= selfRect.width && (selector = "left")
                                        : classList.contains("mud-popover-center-right")
                                            ? deltaToLeft < graceMargin && deltaToRight >= selfRect.width && (selector = "right")
                                            : classList.contains("mud-popover-bottom-left")
                                                ? deltaTop < graceMargin && deltaToRight < graceMargin && deltaBottom >= 0 && deltaToLeft >= selfRect.width
                                                    ? (selector = "bottom-and-left")
                                                    : deltaTop < graceMargin && deltaBottom >= 0
                                                        ? (selector = "bottom")
                                                        : deltaToRight < graceMargin && deltaToLeft >= selfRect.width && (selector = "left")
                                                : classList.contains("mud-popover-bottom-center")
                                                    ? deltaTop < graceMargin && deltaBottom >= 0 && (selector = "bottom")
                                                    : classList.contains("mud-popover-bottom-right") &&
                                                    (deltaTop < graceMargin && deltaToLeft < graceMargin && deltaBottom >= 0 && deltaToRight >= selfRect.width
            ...
            popoverContentNode.classList.contains("mud-popover-fixed") ||
                (window.getComputedStyle(popoverNode).position == "fixed" ? (popoverContentNode.style.position = "fixed") : ((offsetX += window.scrollX), (offsetY += window.scrollY)));
            popoverContentNode.style.left = left + offsetX + "px";
            popoverContentNode.style.top = top + offsetY + "px";
            window.getComputedStyle(popoverNode).getPropertyValue("z-index") != "auto" && ((popoverContentNode.style["z-index"] = window.getComputedStyle(popoverNode).getPropertyValue("z-index")), (popoverContentNode.skipZIndex = !0));
        }
    },
...
class MudPopover {
        ...
        for (const mutation of mutationsList)
            if (mutation.type === "attributes") {
                ...
                if (mutation.attributeName == "class")
                    target.classList.contains("mud-popover-overflow-flip-onopen") && target.classList.contains("mud-popover-open") == !1 && ((target.mudPopoverFliped = null), target.removeAttribute("data-mudpopover-flip")),
                    ...
                    for (let i = 0; i < parent.children.length; i++) {
                        ...
                        tickValue != 0 && (tickValues.indexOf(tickValue) >= 0 || (tickValues.push(tickValue), tickValue > max && (max = tickValue)));
                    }
    ...
    initialize(containerClass, flipMargin) {
        ...
        mainContent.length != 0 &&
            (flipMargin && (window.mudpopoverHelper.flipMargin = flipMargin),
                (this.mainContainerClass = containerClass),
                mainContent[0].mudPopoverMark ||
                ((mainContent[0].mudPopoverMark = "mudded"),
                    this.contentObserver != null && (this.contentObserver.disconnect(), (this.contentObserver = null)),
    ...
    connect(id) {
        ...
        if (popoverNode && popoverNode.parentNode && popoverContentNode) {
            ...
            observer.observe(popoverContentNode, { attributeFilter: ["class", "data-ticks"] });
            ...
            this.map[id] = { mutationObserver: observer, resizeObserver: resizeObserver, contentNodeObserver: contentNodeObserver };
        }
    }
    disconnect(id) {
        if (this.map[id]) {
    ...
    getAllObservedContainers() {
        const result = [];
        for (var i in this.map) result.push(i);
        return result;
    }
}
window.mudPopover = new MudPopover();
window.addEventListener("scroll", () => {
    window.mudpopoverHelper.placePopoverByClassSelector("mud-popover-fixed"), window.mudpopoverHelper.placePopoverByClassSelector("mud-popover-overflow-flip-always");
});
...
class MudResizeListener {
    ...
    resizeHandler() {
        ...
        try {
            this.id
                ? this.dotnet.invokeMethodAsync("RaiseOnResized", { height: window.innerHeight, width: window.innerWidth }, this.getBreakpoint(window.innerWidth), this.id)
                : this.dotnet.invokeMethodAsync("RaiseOnResized", { height: window.innerHeight, width: window.innerWidth }, this.getBreakpoint(window.innerWidth));
        } catch (error) {
...
window.mudResizeListenerFactory = {
    ...
    listenForResize: (dotnetRef, options, id) => {
        var map = window.mudResizeListenerFactory.mapping,
            listener;
        map[id] || ((listener = new MudResizeListener(id)), listener.listenForResize(dotnetRef, options), (map[id] = listener));
    },
    cancelListener: (id) => {
        var map = window.mudResizeListenerFactory.mapping,
            listener;
        map[id] && ((listener = map[id]), listener.cancelListener(), delete map[id]);
    },
    cancelListeners: (ids) => {
        for (let i = 0; i < ids.length; i++) window.mudResizeListenerFactory.cancelListener(ids[i]);
    },
};
class MudResizeObserverFactory {
    ...
    connect(id, dotNetRef, elements, elementIds, options) {
        var existingEntry = this._maps[id],
            observer;
        return existingEntry || ((observer = new MudResizeObserver(dotNetRef, options)), (this._maps[id] = observer)), this._maps[id].connect(elements, elementIds);
    }
    ...
    cancelListener(id) {
        var existingEntry = this._maps[id];
        existingEntry && (existingEntry.cancelListener(), delete this._maps[id]);
    }
}
class MudResizeObserver {
        ...
        this._resizeObserver = new ResizeObserver((entries) => {
            ...
            for (let entry of entries)
                (target = entry.target),
                    ...
                    affectedObservedElement &&
                    ((size = entry.target.getBoundingClientRect()), affectedObservedElement.isInitialized == !0 ? changes.push({ id: affectedObservedElement.id, size: size }) : (affectedObservedElement.isInitialized = !0));
            changes.length > 0 && (this.throttleResizeHandlerId >= 0 && clearTimeout(this.throttleResizeHandlerId), (this.throttleResizeHandlerId = window.setTimeout(this.resizeHandler.bind(this, changes), delay)));
        });
    }
    ...
    connect(elements, ids) {
        var result = [],
            i,
            newEntry;
        for (this.logger("[MudBlazor | ResizeObserver] Start observing elements..."), i = 0; i < elements.length; i++)
            (newEntry = { element: elements[i], id: ids[i], isInitialized: !1 }),
    ...
    disconnect(elementId) {
        ...
        affectedObservedElement &&
            ((element = affectedObservedElement.element),
...
class MudScrollListener {
    ...
    scrollHandler(dotnetReference, event) {
        try {
            ...
            dotnetReference.invokeMethodAsync("RaiseOnScroll", { firstChildBoundingClientRect, scrollLeft, scrollTop, scrollHeight, scrollWidth, nodeName });
        } catch (error) {
...
class MudScrollManager {
    ...
    scrollIntoView(selector, behavior) {
        let element = document.querySelector(selector) || document.documentElement;
        element && element.scrollIntoView({ behavior, block: "center", inline: "start" });
    }
    ...
    handleScroll(dotnetReference, selector) {
        ...
        if (elements.length !== 0) {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                diff < minDifference && ((minDifference = diff), (elementId = element.id));
            }
            if (document.getElementById(elementId).getBoundingClientRect().top < window.innerHeight * 0.8 != !1) {
                ...
                elementId != this.lastKnowElement && ((this.lastKnowElement = elementId), history.replaceState(null, "", window.location.pathname + "#" + elementId), dotnetReference.invokeMethodAsync("SectionChangeOccured", elementId));
            }
        }
    }
    activateSection(sectionId) {
        const element = document.getElementById(sectionId);
        element && ((this.lastKnowElement = sectionId), history.replaceState(null, "", window.location.pathname + "#" + sectionId));
    }
    scrollToSection(sectionId) {
        if (sectionId) {
            let element = document.getElementById(sectionId);
            element && ((this.scrollToSectionRequested = sectionId), element.scrollIntoView({ behavior: "smooth", block: "center", inline: "start" }));
